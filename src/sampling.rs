use burn::tensor::{backend::Backend, DataDataError, DataError, Int, Tensor};
use rand::{
...
impl Sampler {
    ...
    pub async fn sample_async<B: Backend>(&mut self, logits: Tensor<B, 2>) -> Result<Tensor<B, 2, Int>, DataError> {
        match self {
            Self::TopP(s) => s.sample_async(logits).await,
            Self::ArgmaxAsync(s) => logits.argmax(1).into_data_async().await.convert().into(),
            _ => unimplemented!(),
        }
    }
}
